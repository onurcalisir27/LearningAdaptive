...
public:
  explicit TwoLinkControlNode() : Node("two_link_control_node"){
    ...
    this->declare_parameter("u2_bound", 5.0);
   ...
   auto callback_bound1 = [this](const rclcpp::Parameter &p) {
    ...
    MatrixXd Theta_guess(8, 2);
    Theta_guess.setZero();

    Theta_guess(0, 0) = -1.02;   // -θ1(t-1) coefficient → system slightly unstable
    Theta_guess(1, 0) = -0.15;   // -θ2(t-1) coefficient → coupling from joint 2
    Theta_guess(2, 0) = 0.98;    // -θ1(t-2) coefficient → damped return
    Theta_guess(3, 0) = 0.02;    // -θ2(t-2) coefficient
    Theta_guess(4, 0) = 0.002;   // u1(t-1) coefficient → primary control
    Theta_guess(5, 0) = 0.0005;  // u2(t-1) coefficient → weak coupling
    Theta_guess(6, 0) = 0.001;   // u1(t-2) coefficient
    Theta_guess(7, 0) = 0.0002;  // u2(t-2) coefficient

    Theta_guess(0, 1) = -0.20;   // -θ1(t-1) coefficient → coupling from joint 1
    Theta_guess(1, 1) = -1.03;   // -θ2(t-1) coefficient → slightly unstable
    Theta_guess(2, 1) = 0.05;    // -θ1(t-2) coefficient
    Theta_guess(3, 1) = 0.97;    // -θ2(t-2) coefficient → damped return
    Theta_guess(4, 1) = 0.0003;  // u1(t-1) coefficient → weak coupling
    Theta_guess(5, 1) = 0.001;   // u2(t-1) coefficient → primary control (weaker than j1)
    Theta_guess(6, 1) = 0.0001;  // u1(t-2) coefficient
    Theta_guess(7, 1) = 0.0005;  // u2(t-2) coefficient
...
private:
  ...
  void read(const sensor_msgs::msg::JointState::SharedPtr msg){
    ...
    for (size_t i = 0; i < msg->name.size(); ++i) {
      if (msg->name[i] == joint1_name) {
          angle1 = (msg->position[i]);
          // angle1 = wrap(msg->position[i]);
          torque1 = msg->effort[i];
      }
      else if (msg->name[i] == joint2_name) {
          // angle2 = wrap(msg->position[i]);
          angle2 = (msg->position[i]);
          torque2 = msg->effort[i];
      }
    }
