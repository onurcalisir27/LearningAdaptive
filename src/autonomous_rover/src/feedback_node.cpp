...
private:
  ...
  void odom_callbak(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    // Extracting robot pose (Position and Orientation)
  ...
  void message_publisher(){
    ...
    RCLCPP_INFO(this->get_logger(), "X: %f m ", robot_x_.back());
    RCLCPP_INFO(this->get_logger(), "Y: %f m/s", robot_y_.back());
    RCLCPP_INFO(this->get_logger(), "Yaw: %f ", robot_yaw_.back());

    RCLCPP_INFO(this->get_logger(), "---------------------------------");
    RCLCPP_INFO(this->get_logger(), "Robot Velocity:");
    RCLCPP_INFO(this->get_logger(), "Linear X: %f m ", linear_vel_.back());
    RCLCPP_INFO(this->get_logger(), "Angular Z: %f m/s", angular_vel_.back());
